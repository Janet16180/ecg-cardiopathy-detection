...
MIMIC downloads and CODE-15% archives remain outside DVC tracking. The 18.4 GB processed training union, the manifest-backed [100k-plus-labels cohort](sampled-100k-plus-labels-v1.md), and existing frozen experiment inputs are also outside these cache entries. The new cohort refers to local source shards and raw MIMIC records; its seed, manifest hashes and verification receipt preserve selection identity without uploading or duplicating the ECG bytes. Their builders, receipts and hash checks remain the source of truth until a deliberate future migration. Acquisition receipts establish download integrity, not fitness for the clinical endpoint or permission to pool labels. The PTB source ZIP is an unverified archive duplicate and is not tracked; the checked waveform directories and metadata are tracked directly.
