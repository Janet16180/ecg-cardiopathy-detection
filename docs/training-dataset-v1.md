...
PTB development, calibration and test records are absent from every training shard. `heldout_references.csv` preserves their original record/label identity and the existing patient partition: 1,306 development, 564 calibration, 1,896 test. Development/calibration reproduce `ecg_experiment.run.partition_validation`, `GroupShuffleSplit(test_size=0.3, random_state=9001)`. These are **references only**, not a newly processed or evaluated held-out dataset. Fit any downstream normalization only on selected training data.
