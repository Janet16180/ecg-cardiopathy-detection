...
def converted(
    dataset: SampledTrainingECGDataset,
    shards: dict[str, np.ndarray],
    index: int,
) -> tuple[int, np.ndarray, bool]:
    """Read, hash-check, and resample one manifest row."""
    entry = dataset.rows[index]
    if entry["backend"] == "shard":
        signal = np.array(shards[entry["path"]][int(entry["index"])], copy=True)
        validate_signal(signal, require_variable_leads=entry["source"] != "mimic")
        if signal_sha256(signal) != entry["signal_sha256"]:
            raise ValueError(f"Waveform changed: {entry['record_id']}")
    else:
        signal = dataset[index]["signal"]
    constant = entry["source"] == "mimic" and np.any(np.ptp(signal, axis=1) == 0)
    return index, historical_resample(signal), constant
...
def build(output: Path, workers: int) -> None:
    ...
    partial_path = output / "signals.partial.npy"
    ...
    if progress_path.exists():
        ...
        signals = np.load(partial_path, mmap_mode="r+")
    else:
        done = 0
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        for chunk_start in range(done, len(dataset), 256):
            ...
            # Verify each shard once in the main thread. Workers then use only
            # immutable array references, with a per-record waveform hash.
            paths = {dataset.rows[index]["path"] for index in indices
                     if dataset.rows[index]["backend"] == "shard"}
            shards = {path: np.array(dataset._shard(path), copy=True) for path in paths}
            rows = executor.map(partial(converted, dataset, shards), indices)
            for position, (index, waveform, constant) in zip(positions, rows, strict=True):
                ...
                if (position + 1) % 512 == 0 or position + 1 == len(dataset):
                    signals.flush()
                    write_json_atomic(progress_path, {
