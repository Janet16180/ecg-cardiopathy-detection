...
def check_manifests(old_path, old_sha, old_launch_path, old_launch_sha,
    ...
    if any(stage["command"][3] != "scripts.experiments.run_cpc_morphology017_v2"
    ...
    if (len(command) != 8 or command[1:4] != ["-u", "-m", "scripts.coordination.run_priority_queue"]
...
def launch_successor(new_path, new_sha, old_sha, old_pid, old_launch_sha):
    ...
    command = [str(ROOT / ".venv-pretrained/bin/python"), "-u", "-m",
               "scripts.coordination.run_priority_queue", "--manifest", str(new_path.resolve()),
