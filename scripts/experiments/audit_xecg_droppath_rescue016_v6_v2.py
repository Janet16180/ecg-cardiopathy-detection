...
def fixed_set_gradient(model: nn.Module, data: CachedECGs, device: str) -> dict:
    ...
    active = [parameter for parameter in trainable if parameter.grad is not None]
    if not active:
        raise RuntimeError("Fixed-set evaluation pass produced no gradients")
    squared = sum(float(parameter.grad.detach().float().square().sum()) for parameter in active)
    ...
    return {
        ...
        "parameter_tensors_with_gradient": len(active),
        "parameter_tensors_without_gradient": len(trainable) - len(active),
...
def audit(inputs: dict) -> dict:  # noqa: C901 - audit all three frozen phases/arms
    ...
    pre_audit = json.loads((v6.OUT / "pre_audit_arm_hashes.json").read_text())
    if pre_audit["fingerprint"] != inputs["fingerprint"]:
        raise ValueError("Pre-audit arm receipt identity differs")
    for arm in ARMS:
        directory = v6.OUT / arm
        receipt_path = directory / "complete.json"
        if sha256_file(receipt_path) != pre_audit["arms"][arm]["complete_sha256"]:
            raise ValueError(f"{arm} completion receipt changed after frozen pre-audit")
