"""One matched CPC continuation epoch on old versus sampled training ECGs."""
...
from ecg_experiment.cpc_scaling import SampledCPCDataset
...
OUTPUT = ROOT / "outputs/experiment018_cpc_data_scaling"
SEED = 18042
BATCH = 128
LR = 1e-4
PROFILE_BATCHES = 24
...
def input_identity() -> dict[str, str | int | float]:
    ...
    paths = {
        ...
        "adapter_source": ROOT / "ecg_experiment/cpc_scaling.py",
...
def datasets() -> tuple[Dataset, Dataset]:
    ...
    return (PoolDataset(old, old.train_rows, mean, std),
            SampledCPCDataset(COHORT, BASE / "normalization.json"))


def ordered_indices(arm: str, old_count: int, new_count: int) -> np.ndarray:
    """Sample the old pool with replacement or visit each new ECG once."""
    rng = np.random.default_rng(SEED)
    if arm == "old":
        return rng.integers(old_count, size=new_count, dtype=np.int64)
    return rng.permutation(new_count).astype(np.int64)
...
def main() -> None:
    ...
    with gpu_lock("cuda", blocking=False):
        ...
        if args.stage == "profile":
            profile(old, new, identity, args.workers)
            return
