#!/usr/bin/env python3
"""Experiment 016: frozen xECG probe and matched random/probe-head fine-tunes."""
...
DEFAULT_OUTPUT_DIR = ROOT / "outputs/experiment016_xecg_probe_finetune"
...
def cache_fingerprint(cache_dir: Path, raw_dir: Path,
    ...
    current_sources = (sha256_file(ROOT / "scripts/data/prepare_xecg.py"),
                       sha256_file(ROOT / "ecg_experiment/xecg.py"))
    historical_sources = ("4a16e3094b6bc68bacd382607d83f8a683b225a9dd810aab00b83318cda266b9",
    ...
    if source_pair != current_sources:
        refactor = json.loads((ROOT / "outputs/repository_refactor/verification.json").read_text())
        archive = json.loads((ROOT / "outputs/refactor_pause/pause.json").read_text())
        verified = refactor.get("status") == "verified" and refactor.get(
            "source_archive_sha256") == archive.get("source_archive_sha256")
        if source_pair != historical_sources or not verified:
            raise ValueError("xECG cache preprocessing source differs from verified sources")
    if metadata.get("views_sha256") != sha256_file(views_path):
        raise ValueError("xECG cache array differs from its creation receipt")
...
def _save_array(path: Path, values: np.ndarray) -> None:
    """Write a NumPy array atomically without silently changing its extension."""
...
def _source_fingerprint(args: argparse.Namespace, manifest_hashes: dict[str, str],
                        checkpoint_hashes: dict[str, str], cache_hashes: dict[str, Any]) -> dict[str, Any]:
    """Freeze the protocol, sources, model release, cache and patient manifests."""
    result = budget_fingerprint(args, "full", manifest_hashes, checkpoint_hashes, cache_hashes)
    result["protocol_sha256"] = sha256_file(ROOT / "docs/experiment-016-xecg-lpft.md")
    result["experiment"] = "016_xecg_lpft"
    return result


def _load_existing_features(directory: Path, expected_ids: np.ndarray,
                            fingerprint: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    """Verify a completed pooled-feature array against its source receipt."""
    receipt = json.loads((directory / "receipt.json").read_text())
    features_path, ids_path = directory / "features.npy", directory / "ecg_ids.npy"
    if receipt["fingerprint"] != fingerprint:
        raise ValueError("Existing xECG features have another source fingerprint")
    if receipt["sha256"] != {"features.npy": sha256_file(features_path),
                              "ecg_ids.npy": sha256_file(ids_path)}:
        raise ValueError("Existing xECG feature hash mismatch")
    features, ids = np.load(features_path), np.load(ids_path)
    if features.shape != (len(expected_ids), 1024) or not np.array_equal(ids, expected_ids):
        raise ValueError("Existing xECG feature identities or width changed")
    return features, ids, receipt


@torch.inference_mode()
def extract_features(args: argparse.Namespace, rows: Rows, development: Rows,
                     views: np.ndarray, index: dict[int, int],
                     fingerprint: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    """Extract deterministic released-encoder pooled vectors for train and development."""
    directory = args.output_dir / "features"
    receipt_path = directory / "receipt.json"
    features_path, ids_path = directory / "features.npy", directory / "ecg_ids.npy"
    expected_ids = np.array([int(row["ecg_id"]) for row in rows + development], dtype=np.int64)
    if receipt_path.is_file():
        return _load_existing_features(directory, expected_ids, fingerprint)
    if features_path.exists() or ids_path.exists():
        raise FileExistsError("Partial xECG feature cache exists without a receipt")
    from ecg_experiment.xecg import load_xecg

    started = time.monotonic()
    seed_everything(args.seed)
    backbone = load_xecg(args.checkpoint_dir, device=args.device, backend="vanilla",
                         drop_path_prob=DROP_PATH_PROB).eval()
    loader = DataLoader(CachedECGs(views, index, rows + development),
                        batch_size=args.microbatch_size, shuffle=False, num_workers=0)
    chunks = []
    for batch_number, (signal, _) in enumerate(loader):
        pooled, _ = backbone(signal.to(args.device))
        if pooled.shape != (len(signal), 1024) or not torch.isfinite(pooled).all():
            raise RuntimeError("Released xECG produced invalid pooled features")
        chunks.append(pooled.float().cpu().numpy())
        if batch_number % 100 == 0:
            print(json.dumps({"stage": "016_extract", "batches": batch_number + 1}), flush=True)
    if args.device == "cuda":
        torch.cuda.synchronize()
    features = np.concatenate(chunks).astype(np.float32, copy=False)
    if features.shape != (len(expected_ids), 1024):
        raise RuntimeError("xECG feature extraction record count changed")
    _save_array(features_path, features)
    _save_array(ids_path, expected_ids)
    receipt = {"fingerprint": fingerprint, "train_records": len(rows),
               "development_records": len(development), "seconds": time.monotonic() - started,
               "sha256": {"features.npy": sha256_file(features_path),
                           "ecg_ids.npy": sha256_file(ids_path)}}
    write_json_atomic(receipt_path, receipt)
    print(json.dumps({"stage": "016_extract_complete", "seconds": receipt["seconds"]}), flush=True)
    return features, expected_ids, receipt


def affine_from_standardized(coefficient: np.ndarray, intercept: float,
                             mean: np.ndarray, scale: np.ndarray) -> tuple[np.ndarray, float]:
    """Fold a training-only StandardScaler into an affine one-logit head."""
    if not (coefficient.shape == mean.shape == scale.shape) or np.any(scale <= 0):
        raise ValueError("Probe feature statistics or coefficients are malformed")
    raw_weight = coefficient / scale
    return raw_weight, float(intercept - np.dot(raw_weight, mean))


def fit_probe(args: argparse.Namespace, train: Rows, development: Rows, features: np.ndarray,
              fingerprint: dict[str, Any], feature_receipt: dict[str, Any]) -> dict[str, Any]:
    """Fit the fixed regularization grid with train labels and select by dev AUROC."""
    probe_path, receipt_path = args.output_dir / "probe.npz", args.output_dir / "probe.json"
    probe_fingerprint = {"source": fingerprint, "features_sha256": feature_receipt["sha256"],
                         "grid": list(PROBE_C_GRID), "max_iter": 3000}
    if receipt_path.is_file():
        receipt = json.loads(receipt_path.read_text())
        if receipt["fingerprint"] != probe_fingerprint or receipt["probe_sha256"] != sha256_file(probe_path):
            raise ValueError("Existing xECG probe fingerprint or hash differs")
        return receipt
    if probe_path.exists():
        raise FileExistsError("Partial xECG probe exists without a receipt")
    train_x, dev_x = features[:len(train)].astype(np.float64), features[len(train):].astype(np.float64)
    ...
    grid, best_auc, best_model, best_c = [], -1.0, None, None
    started = time.monotonic()
    for c in PROBE_C_GRID:
        candidate = LogisticRegression(C=c, solver="lbfgs", max_iter=3000, random_state=args.seed)
        candidate.fit(scaled_train, train_y)
        auc = float(roc_auc_score(dev_y, candidate.decision_function(scaled_dev)))
        grid.append({"C": c, "development_auroc": auc, "iterations": int(candidate.n_iter_[0])})
        if auc > best_auc:
            best_auc, best_model, best_c = auc, candidate, c
        print(json.dumps({"stage": "016_probe_candidate", **grid[-1]}), flush=True)
    if best_model is None:
        raise RuntimeError("No finite xECG probe candidate")
    coefficient = best_model.coef_[0]
    intercept = float(best_model.intercept_[0])
    raw_weight, raw_bias = affine_from_standardized(coefficient, intercept,
                                                    scaler.mean_, scaler.scale_)
    expected = best_model.decision_function(scaled_dev)
    actual = dev_x @ raw_weight + raw_bias
    max_difference = float(np.max(np.abs(actual - expected)))
    if not np.allclose(actual, expected, rtol=1e-10, atol=1e-9):
        raise RuntimeError("Folded raw-feature probe logits differ from standardized probe")
    write_npz_atomic(probe_path, coefficient=coefficient, intercept=np.array(intercept),
                     mean=scaler.mean_, scale=scaler.scale_, raw_weight=raw_weight,
                     raw_bias=np.array(raw_bias))
    receipt = {"fingerprint": probe_fingerprint, "selected_C": best_c,
               "development_auroc": best_auc, "grid": grid, "seconds": time.monotonic() - started,
               "max_affine_difference": max_difference, "probe_sha256": sha256_file(probe_path)}
    write_json_atomic(receipt_path, receipt)
    return receipt


def _load_probe_head(model: nn.Module, probe_path: Path, dev_features: np.ndarray) -> float:
    """Transfer the fitted affine classifier and verify float32 PyTorch logits."""
    with np.load(probe_path) as probe:
        weight = np.asarray(probe["raw_weight"], dtype=np.float32)
        bias = float(probe["raw_bias"])
        standardized = ((dev_features.astype(np.float64) - probe["mean"]) / probe["scale"])
        expected = standardized @ probe["coefficient"] + float(probe["intercept"])
    with torch.no_grad():
        model.head.weight.copy_(torch.from_numpy(weight[None]).to(model.head.weight.device))
        model.head.bias.fill_(bias)
        actual = model.head(torch.from_numpy(dev_features).to(model.head.weight.device)).squeeze(-1)
        actual = actual.cpu().numpy()
    difference = float(np.max(np.abs(actual - expected)))
    if not np.allclose(actual, expected, rtol=2e-5, atol=2e-4):
        raise RuntimeError(f"Transferred xECG head logits differ from probe: {difference}")
    return difference


def profile_full_pass(args: argparse.Namespace, train: Rows, development: Rows,
                      views: np.ndarray, index: dict[int, int],
                      fingerprint: dict[str, Any]) -> dict[str, Any]:
    """Measure two complete train/evaluation/checkpoint passes on the real cache."""
    path = args.output_dir / "profile.json"
    if path.is_file():
        saved = json.loads(path.read_text())
        if saved["fingerprint"] != fingerprint:
            raise ValueError("Existing full-pass profile belongs to other inputs")
        return saved
    model, optimizer, scheduler = build_classifier(args, len(train))
    generator = torch.Generator().manual_seed(args.seed)
    train_loader = DataLoader(CachedECGs(views, index, train), batch_size=args.microbatch_size,
                              shuffle=True, generator=generator, num_workers=0)
    dev_loader = DataLoader(CachedECGs(views, index, development),
                            batch_size=args.microbatch_size, num_workers=0)
    passes = []
    profile_checkpoint = args.output_dir / "profile_checkpoint.pt"
    for pass_number in range(2):
        started = time.monotonic()
        loss, updates = train_epoch(model, train_loader, optimizer, scheduler,
                                    args.device, args.effective_batch_size)
        dev_logits = predict(model, dev_loader, args.device)
        if not np.isfinite(dev_logits).all():
            raise RuntimeError("Profile development logits are nonfinite")
        write_torch_atomic(profile_checkpoint, {"model": cpu_state(model),
                                                "optimizer": optimizer.state_dict(),
                                                "scheduler": scheduler.state_dict(),
                                                "fingerprint": fingerprint})
        restored = torch.load(profile_checkpoint, map_location="cpu", weights_only=True)
        if restored["fingerprint"] != fingerprint:
            raise RuntimeError("016 profile checkpoint fingerprint changed on reload")
        for name, parameter in cpu_state(model).items():
            if not torch.equal(parameter, restored["model"][name]):
                raise RuntimeError(f"016 profile checkpoint parameter changed on reload: {name}")
        if len(restored["optimizer"]["state"]) != len(optimizer.state_dict()["state"]):
            raise RuntimeError("016 profile optimizer state incomplete on reload")
        if args.device == "cuda":
            torch.cuda.synchronize()
        pass_result = {"pass": pass_number + 1, "seconds": time.monotonic() - started,
                       "loss": loss, "updates": updates,
                       "development_auroc": float(roc_auc_score(
                           [int(row["target"]) for row in development], dev_logits))}
        passes.append(pass_result)
        print(json.dumps({"stage": "016_profile_pass", **pass_result}), flush=True)
    receipt = {"fingerprint": fingerprint, "passes": passes,
               "checkpoint_sha256": sha256_file(profile_checkpoint),
               "peak_cuda_memory_bytes": peak_gpu_bytes(args.device),
...
def _cost_gate(args: argparse.Namespace, profile: dict[str, Any],
               extraction: dict[str, Any], probe: dict[str, Any]) -> dict[str, float]:
    """Stop if measured extraction/profile plus projected paired runs exceed two hours."""
    slow_pass = max(row["seconds"] for row in profile["passes"])
    measured = extraction["seconds"] + sum(row["seconds"] for row in profile["passes"])
    projected = measured + probe["seconds"] + 4 * slow_pass + 300.0
    result = {"measured_extraction_profile_seconds": measured,
              "slow_complete_pass_seconds": slow_pass,
              "projected_total_seconds": projected, "ceiling_seconds": 7200.0}
    write_json_atomic(args.output_dir / "cost_gate.json", result)
    if projected > 7200:
        raise RuntimeError(f"Experiment 016 projected suite exceeds two-hour gate: {projected:.0f}s")
    return result


def train_arm(args: argparse.Namespace, arm: str, train: Rows, development: Rows,
              views: np.ndarray, index: dict[int, int], dev_features: np.ndarray,
              fingerprint: dict[str, Any], probe_sha256: str) -> dict[str, Any]:
    """Run one of the matched two-epoch fine-tunes, selecting on development only."""
    directory = args.output_dir / arm
    arm_fingerprint = {"source": fingerprint, "arm": arm, "probe_sha256": probe_sha256}
    completed = directory / "complete.json"
    if completed.is_file():
        receipt = json.loads(completed.read_text())
        if receipt["fingerprint"] != arm_fingerprint:
            raise ValueError("Completed 016 arm has another fingerprint")
        if receipt["sha256"] != artifact_hashes(
                directory, ("model.pt", "history.json", "development_logits.npz", "config.json")):
            raise ValueError("Completed 016 arm has altered artifacts")
        return receipt
    model, optimizer, scheduler = build_classifier(args, len(train))
    transfer_difference = None
    if arm == "probe_head":
        transfer_difference = _load_probe_head(model, args.output_dir / "probe.npz", dev_features)
    generator = torch.Generator().manual_seed(args.seed)
    train_loader = DataLoader(CachedECGs(views, index, train), batch_size=args.microbatch_size,
                              shuffle=True, generator=generator, num_workers=0)
    dev_loader = DataLoader(CachedECGs(views, index, development),
                            batch_size=args.microbatch_size, num_workers=0)
    directory.mkdir(parents=True, exist_ok=True)
    config = {"fingerprint": arm_fingerprint, "seed": args.seed, "epochs": args.epochs,
              "effective_batch_size": args.effective_batch_size,
              "microbatch_size": args.microbatch_size, "core_lr": CORE_LR,
              "head_lr": HEAD_LR, "weight_decay": WEIGHT_DECAY,
              "layerwise_lr_decay": LAYERWISE_LR_DECAY,
              "transfer_logit_max_abs_difference": transfer_difference}
    write_json_atomic(directory / "config.json", config)
    result = fit(model, optimizer, scheduler, train_loader, dev_loader,
                 np.array([int(row["target"]) for row in development]), directory,
                 arm_fingerprint, generator, args.device, args.epochs, args.patience,
                 args.effective_batch_size, resume=(directory / "resume.pt").is_file())
    logits = predict(model, dev_loader, args.device)
    write_npz_atomic(directory / "development_logits.npz",
                     ecg_ids=np.array([int(row["ecg_id"]) for row in development]), logits=logits)
    write_torch_atomic(directory / "model.pt", {"model": cpu_state(model),
    ...
    config.update({"best_epoch": result["best_epoch"],
                   ...
                   "peak_cuda_memory_bytes": peak_gpu_bytes(args.device)})
    write_json_atomic(directory / "config.json", config)
    files = ("model.pt", "history.json", "development_logits.npz", "config.json")
    receipt = {"fingerprint": arm_fingerprint, "sha256": artifact_hashes(directory, files)}
    write_json_atomic(completed, receipt)
    (directory / "resume.pt").unlink(missing_ok=True)
    return receipt


def parse_016_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse the frozen full-label 016 pilot options."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--stage", choices=("check", "extract", "probe", "profile", "train", "all"),
                        default="all")
    parser.add_argument("--manifest-root", type=Path, default=DEFAULT_MANIFEST_ROOT)
    parser.add_argument("--raw-dir", type=Path, default=DEFAULT_RAW_DIR)
    parser.add_argument("--checkpoint-dir", type=Path, default=DEFAULT_CHECKPOINT_DIR)
    ...
    parser.add_argument("--threads", type=int, default=1)
    args = parser.parse_args(argv)
    if (args.seed, args.epochs, args.patience, args.microbatch_size,
            args.effective_batch_size, args.threads) != (42, 2, 2, 16, 64, 1):
        parser.error("016 requires the frozen seed, two epochs, batch sizes and thread count")
    args.bootstrap = 0
    return args


def _run_training(args: argparse.Namespace, train: Rows, development: Rows,
                  views: np.ndarray, index: dict[int, int], features: np.ndarray,
                  fingerprint: dict[str, Any], extraction: dict[str, Any],
                  probe: dict[str, Any]) -> None:
    """Enforce the measured cost gate and execute both fine-tuning arms."""
    profile = json.loads((args.output_dir / "profile.json").read_text())
    if profile["fingerprint"] != fingerprint:
        raise ValueError("016 profile has another input fingerprint")
    _cost_gate(args, profile, extraction, probe)
    with gpu_lock(args.device, blocking=False):
        for arm in ("random_head", "probe_head"):
            train_arm(args, arm, train, development, views, index,
                      features[len(train):], fingerprint, probe["probe_sha256"])


def main(argv: list[str] | None = None) -> None:
    """Check frozen inputs, then extract, probe, profile and train as requested."""
    args = parse_016_args(argv)
    if args.stage in {"extract", "profile", "train", "all"}:
        require_cuda(args.device)
    torch.set_num_threads(args.threads)
    rows, development, _, manifest_hashes = load_manifests(args.manifest_root, "full")
    ...
    check_cache_coverage({"full": (rows, development, [], manifest_hashes)}, index)
    fingerprint = _source_fingerprint(args, manifest_hashes, checkpoint_hashes, cache_hashes)
    args.output_dir.mkdir(parents=True, exist_ok=True)
    write_json_atomic(args.output_dir / "input_fingerprint.json", fingerprint)
    if args.stage == "check":
        print(json.dumps({"stage": "016_check", "records": len(rows["labeled_train"]),
                          "development_records": len(development), "fingerprint": fingerprint}), flush=True)
        return
    if args.stage in {"extract", "all"}:
        with gpu_lock(args.device, blocking=False):
            extract_features(args, rows["labeled_train"], development, views, index, fingerprint)
    if args.stage in {"probe", "profile", "train", "all"}:
        if not (args.output_dir / "features/receipt.json").is_file():
            raise FileNotFoundError("Extract xECG features before probe, profile or training")
        features, _, extraction = extract_features(
            args, rows["labeled_train"], development, views, index, fingerprint)
    if args.stage in {"probe", "all"}:
        fit_probe(args, rows["labeled_train"], development, features, fingerprint, extraction)
    if args.stage in {"profile", "train", "all"}:
        probe = fit_probe(args, rows["labeled_train"], development, features, fingerprint, extraction)
    if args.stage in {"profile", "all"}:
        with gpu_lock(args.device, blocking=False):
            profile_full_pass(args, rows["labeled_train"], development, views, index, fingerprint)
    if args.stage in {"train", "all"}:
        _run_training(args, rows["labeled_train"], development, views, index,
                      features, fingerprint, extraction, probe)
