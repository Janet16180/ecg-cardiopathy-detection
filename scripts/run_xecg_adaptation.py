...
from scripts import run_xecg_finetune as ft
...
def load_ssl_cache(path: Path):
    ...
    from scripts.prepare_xecg_ssl import FIELDS, selected_rows
...
def source_identity():
    files = ("ecg_experiment/xecg_adaptation.py", "scripts/run_xecg_adaptation.py",
             "ecg_experiment/xecg.py", "scripts/run_xecg_finetune.py",
             "scripts/prepare_xecg_ssl.py", "ecg_experiment/run.py", "ecg_experiment/evaluation.py",
             "scripts/report_xecg_adaptation.py", "scripts/run_cpc_experiment.py")
    ...
    with ft.gpu_lock(args.device):
        ...
        if args.stage in ("train", "all"):
            ...
            if all((args.output_dir / "transfer" / arm / f"xecg_{budget}_seed42" / "complete.json").is_file()
                ...
                from scripts.report_xecg_adaptation import report
