...
def main():
    ...
    if predecessor and b"scripts.run_cpc_coordinated" not in predecessor[1]:
        raise RuntimeError("Predecessor is not the expected ECG suite")
    if mimic and b"scripts.run_mimic_scale" not in mimic[1]:
        raise RuntimeError("MIMIC PID belongs to an unexpected process")
    if preparation and b"scripts.prepare_xecg" not in preparation[1]:
    ...
    try:
        ...
        if not (ROOT / "data/processed/ptbxl/xecg_views/metadata.json").exists():
            command = [sys.executable, "-u", "-m", "scripts.run_xecg_finetune",
