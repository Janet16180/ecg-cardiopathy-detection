...
def test_center_crop_is_machine_readable_ssl_only(tmp_path: Path) -> None:
    ...
    assert row["record_annotation_available"] == "false"
    assert row["endpoint_supervised_eligible"] == "false"
    assert row["overlap_by_record"] == "true" and row["overlap_by_signal"] == "true"


def test_changed_raw_file_fails_without_publishing_output(tmp_path: Path) -> None:
...
def test_changed_canonical_view_fails_without_publishing_output(tmp_path: Path) -> None:
    root, _, prepared, _, signal = fixture(tmp_path)
    output = root / "data/processed/views/strict"
