...
from scripts.experiments.audit_xecg_droppath_rescue016_v6_v2 import fixed_set_gradient
...
def test_fixed_training_subset_eval_gradient_and_hypothetical_clip():
    ...
    assert diagnostic["trainable_parameter_tensors"] == 2
    assert diagnostic["parameter_tensors_with_gradient"] == 1
    assert diagnostic["parameter_tensors_without_gradient"] == 1
